...
function showQuickAdd(draft) {
  ...
  `;
    const form = query(shadow, "form");
    const page = query(shadow, ".page");
    const taskName = query(shadow, ".task-name");
    const dueDate = query(shadow, ".due-date");
    const notes = query(shadow, ".notes");
    const error = query(shadow, ".error");
    const timer = query(shadow, ".timer");
    ...
    const cleanup = (discardScreenshot) => {
    ...
    query(shadow, ".close").addEventListener("click", onCancel);
    query(shadow, ".cancel").addEventListener("click", onCancel);
...
function showToast(message) {
    ...
    <div class="toast"></div>
  `;
    query(shadow, ".toast").textContent = message;
    document.documentElement.append(host);
